...
but we skip definition of capacitance parameters etc. For this, directly reading the tutorials is best.
...
that govern the capacitances between the 9 gates and 6 dots, as well as the affect the 9 gates have on the barrier potential and therefore the tunnel coupling.
...
parameters, the capacitanca matrices between plunger gates and dots,  ``C_DG``, the interdot capacitances ``C_DD``,
...
The capacitance simulation is purely classical. It can only be used to query the ground state of the electron configuration given a set of gate voltages, 
...
in our case 9 values, and and :math:`m` and :math:`P` are projection parameters, where :math:`m` is a 9 dimensional vector and :math:`P` a 9x2 matrix. 
...
The resulting grid is a rastered low resolution image (TODO add image). We can improve on this by plotting the exact boundaries between the two ground states using ``get_polytopes`` and ``plot_polytopes``::
...
----------------------------------
Now that we are able to generate a CSD, we want to also simulate tunneling and simulate a more realistic sensor signal (todo: explain)::
    ...
    tunneling_sim = ApproximateTunnelingSimulator(capacitive_sim, #the underlying polytope simulation
                                                 ...
                                                 temperature, #device temperature, should be <=100mK
...
The last plot shos that most of the sensor signal is very low and there is only a small signal with a peak. This is because of the missing sensor compensation: the sensor peak
...
QDarts makes use of this in all its transformation code. For example, compensating the sensor dot::
    ...
    compensated_tsim, compensation_transform = compensate_simulator_sensors(
        tunneling_sim, #the simulator object to compensate
        target_state=target_state, #the electron configuration around which to compensate
        compensation_gates= sensor_plungers, #the ids of the sensor gate plungers. Here: [6,7,8]
        sensor_ids = sensor_dots,#the ids of the sensor dots. Here: [3,4,5], 
        sensor_detunings = np.array([-0.005,-0.005,-0.005]) #shift of the sensor dot plungers
    )
    m_comp = compensation_transform(m)
